...
from utils.defs import info_pbvoto, loc_votos 
from core.carregar import load_corr
...
def grafico2(df):
    ...
    df = df.groupby(['RPA'], as_index=False).agg(
        Artistas = ('inscritos','sum'),
        conv_social = ('conv_social','sum'),
        votos_bairro = ('votos','sum')
    )    
    cores_itens = {
        "RPA4": "#f9c393", 
        "RPA6": "#0062ff"} 
...
with col2:
    st.plotly_chart(grafico2(df_bairros), use_container_width=True)
